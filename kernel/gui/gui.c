...
UINT32 ColorTable[16] = {0x000000,0x0000AA,0x00AA00,0x00AAAA,0xAA0000,0xAA00AA,\
...
VOID PutPixelBuff(UINTN X, UINTN Y, UINTN Color,VOID *Buffer)
{
	...
	*(UINT32*)((UINT32*)buf + (G->ByteScanLine * Y) + X) = Color;
				...
				VOID *Buffer)
{
	...
	if(C == '\t'){
		...
		if(C == 'i' || C == 'I' || C == 'l' || C =='!')
				Hand->Font.RSizeX = Hand->Font.RSizeX + 4; // Nelson isto so funciona com o DrawCharTransparent
						...
						UINTN Flag)
{
	...
	//Font
	InitializeBoxDraw(GwHand,12,16,0xFFFF55,0,font12x16);
